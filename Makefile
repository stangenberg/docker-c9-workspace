...
CONTAINER_NAME=c9-workspace
DOCKER_IMAGE=c9-workspace
DOCKER_TAG=latest
...
# normal run
run: clean
	docker run --privileged --name="$(CONTAINER_NAME)" $(DOCKER_USER)/$(DOCKER_IMAGE):$(DOCKER_TAG)

# run to bash 
bash: clean
	docker run -t -i --privileged --name="$(CONTAINER_NAME)" $(DOCKER_USER)/$(DOCKER_IMAGE):$(DOCKER_TAG) /sbin/my_init -- zsh -l
