...
# normal run
run: clean
	docker run --name="$(CONTAINER_NAME)" -p 2202:22 $(DOCKER_USER)/$(DOCKER_IMAGE):$(DOCKER_TAG)

# run to bash 
bash: clean
	docker run -t -i --name="$(CONTAINER_NAME)" -p 2202:22 $(DOCKER_USER)/$(DOCKER_IMAGE):$(DOCKER_TAG) /sbin/my_init -- zsh -l
